...
import com.techram.model.WeatherInfo;
import com.techram.model.WeatherPreferences;
import com.techram.model.YahooWeatherHelper;
import com.techram.view.ContextMenuAdapter;
import com.techram.view.ContextMenuItem;
import com.google.ads.*;

/*******************************************************************************
 * Purpose of this class for Activity setting yahoo weather service First of all
 * with this requirement i think Widget is better (From user view) but currently
 * i have a litter free time, so i create only Activity version
 * 
 * I hope we will have widget version in near future when i have enough free
 * time
 * 
 * @author DatNQ
 * 
...
public class ActivityWeatherSetting extends Activity {
   ...
   public void onDestroy() {
	...
	protected Dialog onCreateDialog(int id) {
		...
		builder.setPositiveButton(R.string.str_agree,
		...
		builder.setNegativeButton(R.string.str_disagree,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						System.exit(0);
					}
	...
	private void updateDataOfCurrentLocation() {
		...
		String strRegURL = String.format(URL_YAHOO_API_WEATHER, strWOEID);

		ShowProgress = ProgressDialog.show(ActivityWeatherSetting.this, "",
				"Loading. Please wait...", true);
		new loadingTask().execute(strRegURL);
	}
	...
	private void drawWeatherScreen() {
		...
		/*
		 * if (m_Preferneces.getMyCvSetting() == true){ displayCV(); }
		 */
	}
	...
	private void updateWeatherInfo(WeatherInfo weatherInfo) {
		...
		String strTemp = weatherInfo
				.getTemperature(WeatherInfo.TEMPERATURE_FMT_CELSIUS);
		if (bIsC == true) {
	...
	class loadingTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String s) {
			...
			try {
				...
				WeatherInfo yahooWeather = YahooWeatherHelper
						.parserYahooWeatherInfo(db.parse(is));
				updateWeatherInfo(yahooWeather);
				notifyUpdateTime();
